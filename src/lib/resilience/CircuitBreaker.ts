...
export class CircuitBreaker {
    ...
    constructor(
        ...
        logger.info(`Circuit breaker initialized: ${this.name} ${JSON.stringify({
    ...
    private onFailure(error: any): void {
        ...
        if (this.isExpectedError(error)) {
            logger.debug(`Expected error in circuit breaker ${this.name}, not counting as failure`);
            return;
        }

        logger.warn(`Circuit breaker failure ${this.consecutiveFailures}/${this.options.failureThreshold}: ${this.name} ${JSON.stringify({
    ...
    private checkState(): void {
        if (this.state === CircuitBreakerState.OPEN && this.nextAttempt && Date.now() >= this.nextAttempt) {
    ...
    private changeState(newState: CircuitBreakerState): void {
        ...
        logger.info(`Circuit breaker state change: ${this.name} ${oldState} -> ${newState}`);
    }
